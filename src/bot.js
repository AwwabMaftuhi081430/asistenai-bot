...
if (!token) {
  console.error('[FATAL] BOT_TOKEN tidak ditemukan!');
...
function createBot() {
  ...
  setNotifyTimeout(async (chatId) => {
    try {
      await bot.telegram.sendMessage(chatId, '⏰ Sesi habis. Silakan ketik perintah kembali.', mainKeyboard());
    } catch (_) {}
  });
  ...
  // Error middleware
  bot.catch((err, ctx) => errorHandler(err, ctx));
...
const defaultBot = token ? createBot() : null;

module.exports = { bot: defaultBot, createBot, registerHandlers };
